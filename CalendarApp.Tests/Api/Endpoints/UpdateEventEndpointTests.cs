...
public class UpdateEventEndpointTests
{
    ...
    public async Task HandleAsync_Should_Update_Event_When_Valid()
    {
        ...
        var existingEvents = new List<CalendarEventModel> { existing };
        ...
        var endpoint = Factory.Create<UpdateEventEndpoint>(_repositoryMock.Object, _eventModelValidator, _eventIdValidator);
    ...
    public async Task HandleAsync_Should_Return_400_When_Model_Is_Invalid()
    {
        ...
        Assert.Contains(endpoint.ValidationFailures, v =>
            v.ErrorMessage.Contains("Title is required") || v.ErrorMessage.Contains("End date must be after the start date"));
